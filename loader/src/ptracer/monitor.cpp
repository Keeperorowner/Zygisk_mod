        ...
        for (;;) {
            ...
            switch (msg.cmd) {
                ...
                case SYSTEM_SERVER_STARTED:
                    ...
                    if (mount(prop_path.c_str(), "/data/adb/modules/Zygisk_mod/module.prop", nullptr, MS_BIND, nullptr) == -1) {
...
static bool ensure_daemon_created(bool is_64bit) {
    ...
    if (is_64bit) {
        ...
        umount2("/data/adb/modules/Zygisk_mod/module.prop", MNT_DETACH);
    }
...
static void updateStatus() {
    ...
    if (tracing_state != TRACING && !monitor_stop_reason.empty()) {
        ...
        status_text += ")";
        out += "stoped:"+monitor_stop_reason;
    }

#define WRITE_STATUS_ABI(suffix) \
    if (status##suffix.supported) { \
        if (tracing_state != TRACING) {state1=1;} \
        else if (status##suffix.zygote_injected) {state1=0;} \
        else {state1=2;} \
    ...
    fprintf(info.get(),"{\"a\":%d,\"b\":%d,\"p32\":%d,\"p64\":%d,\"in32\":\"%s\",\"in64\":\"%s\",\"out\":\"%s\"}",state,state1,state32,state64,in32.c_str(),in64.c_str(),out.c_str());
}

static bool prepare_environment() {
    prop_path = zygiskd::GetTmpPath() + "/module.prop";
    info_path = "/data/adb/modules/Zygisk_mod/info.json";
